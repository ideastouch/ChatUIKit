...
class ChatViewController: UIViewController, UITextViewDelegate {
    ...
    var navigationTitle = "Messages"
    var dataSource: UITableViewDataSource? { didSet {
        if let _ = self.viewIfLoaded, let tableView = self.tableView {
            tableView.dataSource = self.dataSource } } }
    var delegate: UITableViewDelegate? { didSet {
        if let _ = self.viewIfLoaded, let tableView = self.tableView {
            tableView.delegate = self.delegate } } }
    var sendMessageBlock: ((_ message:String, _ complete:@escaping ()->Void)->Void)?
    ...
    @IBAction func tapGestureRecognizerViewAction(_ sender: Any) {
